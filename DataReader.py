...
class DataReader:
    ...
    ### refactored mathod: this method extracts the tweets and their subtask A labels to create the training dataset
    ...
    ## implemented method: gets the test data and their labels to create the test dataset
    def get_labelled_test_data(self):
        data = []
        labels = []
        with open(self.file_path,encoding='utf8') as tsvfile:
            reader = csv.reader(tsvfile, delimiter='\t')
            for i,line in enumerate(tqdm(reader,'Reading Test Data')):
                ...
                if  self.sub_task:
                    self.filter_subtask(data,labels,line[0],label)
                else:
                    data.append(line[0])
                    labels.append(label)
    ...
    ### refactored method that extracts test tweets and theri ids
    ...
    ### refactored method: this method implements random shuffling of the training data, then extracts the tweets and their labels 
    ### based on the subtask specified by the caller method 
    ...
    ### refactored method: this method performs data augmentation: method not used    
    ...
    ### refactored method: this method creates integer labels from str input
    ### for subtask A:
        # the label NOT gets label 0
        # the label OFF gets label 1
    ...
    ### refactored method: thsi method extracts appropraite data nd label for specific subtask
